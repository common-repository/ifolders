...
class HelperModel {
    ...
    public static function getTableName( $table ) {
        global $wpdb;
        return $wpdb->prefix . IFOLDERS_PLUGIN_DB_TABLE_PREFIX . '_' . $table;
    }

    public static function disableNotice() {
        return update_option( 'ifolders_dismiss_first_use_notification', true, false );
    }

    public static function uninstall() {
        include_once( ABSPATH . 'wp-admin/includes/plugin.php' );

        if ( \is_plugin_active( IFOLDERS_PLUGIN_BASE_NAME ) ) {
            \deactivate_plugins( IFOLDERS_PLUGIN_BASE_NAME );
    ...
    public static function getMediaHoverDetails() {
        ...
        $data['items'][] = [ 'id' => 'title', 'title' => esc_html__( "Title", 'ifolders' ) ];
        $data['items'][] = [ 'id' => 'alternative_text', 'title' => esc_html__( "Alternative text", 'ifolders' ) ];
        $data['items'][] = [ 'id' => 'file_url', 'title' => esc_html__( "File URL", 'ifolders' ) ];
        $data['items'][] = [ 'id' => 'dimension', 'title' => esc_html__( "Dimension", 'ifolders' ) ];
        $data['items'][] = [ 'id' => 'size', 'title' => esc_html__( "Size", 'ifolders' ) ];
        $data['items'][] = [ 'id' => 'filename', 'title' => esc_html__( "Filename", 'ifolders' ) ];
        $data['items'][] = [ 'id' => 'type', 'title' => esc_html__( "Type", 'ifolders' ) ];
        $data['items'][] = [ 'id' => 'date', 'title' => esc_html__( "Date", 'ifolders' ) ];
        $data['items'][] = [ 'id' => 'uploaded_by', 'title' => esc_html__( "Uploaded by", 'ifolders' ) ];
    ...
    public static function getMessagesForSidebar() {
        ...
        $messages = [
            ...
            'upgrade' => esc_html__( "This is the pro feature.", 'ifolders' ) . " <a target='_blank' href='{$upgrade_url}' rel='noreferrer'>" . esc_html__( "Upgrade to get the full power.", 'ifolders' ) . '</a>',
            'parent_folder' => esc_html__( "Parent Folder", 'ifolders' ),
            'new_folder' => esc_html__( "New Folder", 'ifolders' )
        ];
        return $messages;
    }
    ...
    public static function getMessagesForSettings() {
        ...
        $messages = [
            'success' => esc_html__( "The operation completed successfully", 'ifolders' ),
            'failed'  => esc_html__( "The operation failed", 'ifolders' ),
            'builtin' => esc_html__( "These plugin settings are built-in and cannot be modified", 'ifolders' ),
            'upgrade' => esc_html__( "This is the pro feature.", 'ifolders' ) . " <a target='_blank' href='{$upgrade_url}' rel='noreferrer'>" . esc_html__( "Upgrade to get the full power.", 'ifolders' ) . '</a>',
            'status' => [
    ...
    public static function getTemplate( $name ) {
        if ( $name ) {
            $file = IFOLDERS_PLUGIN_PATH . '/includes/Views/' . $name . '.php';
            $data = null;
    ...
    public static function getContextMenu() {
        return [
            [
                'id' => 'create',
                'right' => 'c',
                'title' => esc_html__( "New Folder", 'ifolders' ),
            ...
            [
                'id' => 'copy',
                'right' => 'e',
                'title' => esc_html__( "Copy", 'ifolders' ),
            ...
            [
                'id' => 'paste',
                'right' => 'e',
                'title' => esc_html__( "Paste", 'ifolders' ),
            ...
            [
                'id' => 'download',
                'right' => 'v',
                'title' => esc_html__( "Download", 'ifolders' ),
