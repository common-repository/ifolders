...
class FoldersModel {
    public static function getUnregisteredTypes() {
        $data['items'] = [];
        $data['items'][] = [ 'id' => 'attachment', 'title' => esc_html__( "Media", 'ifolders' ) ];
        $data['items'][] = [ 'id' => 'post', 'title' => esc_html__( "Posts", 'ifolders' ) ];
        $data['items'][] = [ 'id' => 'page', 'title' => esc_html__( "Pages", 'ifolders' ) ];
        $data['items'][] = [ 'id' => 'users', 'title' => esc_html__( "Users", 'ifolders' ) ];
    ...
    public static function getDownloadFoldersUrl( $type, $ids ) {
        ...
        if ( $rights && $rights['access_type'] && $rights['v'] ) {
            if ( is_array( $ids ) && count( $ids ) > 0 ) {
                ...
                return rest_url( IFOLDERS_PLUGIN_REST_URL . '/folders/download/' . $transientId, is_ssl() ? 'https' : 'http' );
            }
        }
    ...
    public static function downloadFolders( $transientId ) {
        ...
        $filename = 'ifolders-' . gmdate('d-m-Y') . '-' . uniqid() . '.zip';
